...
def calculate_fine_for_violations(selected_violations, vehicle_type):
    """Υπολογισμός προστίμου για επιλεγμένες παραβάσεις"""
...
def submit_violation():
    ...
    try:
        ...
        # Υπολογισμός προστίμων για τις επιλεγμένες παραβάσεις
        fine_calculation = calculate_fine_for_violations(selected_violations, vehicle_type)
        ...
        violation = Violation(
            ...
            # Νέα πεδία προστίμων
            violation_articles=json.dumps(fine_calculation['violation_articles']),
            total_fine_amount=fine_calculation['total_fine'],
            fine_breakdown=json.dumps(fine_calculation['fine_breakdown']),
...
def admin_dashboard():
    ...
    try:
        ...
        # Συνολικό ποσό προστίμων
        try:
            total_fines_amount = db.session.query(
                db.func.sum(Violation.total_fine_amount)
            ).filter(Violation.total_fine_amount.isnot(None)).scalar() or 0
        except Exception:
...
def admin_edit_violation(violation_id):
    ...
    if request.method == 'POST':
        try:
            ...
            # Επανυπολογισμός προστίμων
            fine_calculation = calculate_fine_for_violations(selected_violations, violation.vehicle_type)
            violation.violation_articles = json.dumps(fine_calculation['violation_articles'])
            violation.total_fine_amount = fine_calculation['total_fine']
            violation.fine_breakdown = json.dumps(fine_calculation['fine_breakdown'])
